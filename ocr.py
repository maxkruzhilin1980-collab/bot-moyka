...
def _prepare(text: str) -> str:
    text = text or ""
    text = re.sub(r"ндс.{0,12}", " ", text, flags=re.I)
...
def _money_from_text(text: str) -> list[float]:
    found = []
    for raw in re.findall(r"(?<!\d)(\d{3,5}[.,]\d{2})(?!\d)", text or ""):
        val = _num(raw)
        if val is not None:
            found.append(val)
    return found
...
def _pick_amount(text: str, items: list[dict]) -> float | None:
    ...
    from_items = [
        float(i["amount"])
        for i in items
        if 100 <= float(i.get("amount") or 0) <= 99999
    ]
    from_text = _money_from_text(clean)
    ...
    for key in ("итого", "итог", "total"):
        ...
        if pos < 0:
            continue
        for val in _money_from_text(clean[pos : pos + 24]):
            return val
